...
function browser_longname( $short ){
	global $a_browser_types;
	foreach($a_browser_types as $type){
		if( $type[2] == $short ){
			return $type[0];
		}
	}
...
function browser_set( $acctid = 0, $browserinfo = false ){
	global $session;
	if( !$acctid ){
		$acctid = $session['user']['acctid'];
	}
	if( $browserinfo === false ){
		$browserifo = browser_detection( 'full_assoc', 3 ); 
	}
	db_query( 'UPDATE account_stats SET browser="'.$browserifo['browser_working'].'", browser_version="'.$browserifo['browser_number'].'" WHERE acctid='.$acctid );
}
...
function browser_detection( $which_test, $test_excludes='', $external_ua_string='' ) 
{
	...
	*/
	if ( !$b_repeat ) {
		...
		mb_strstr instead of mb_stristr, which drops overhead slightly.
		*/
		if ( $external_ua_string ) {
			$browser_user_agent = mb_strtolower( $external_ua_string );
		}
		...
		*/
		...
		for ( $i = 0; $i < $i_count; $i++ ) {
			...
			if ( mb_strstr( $browser_user_agent, $browser_temp ) ) {
				/*
				...
				switch ( $browser_working ) {
					...
					case 'moz':
						...
						*/
						...
						for ( $j = 0; $j < $j_count; $j++ ) {
							if ( mb_strstr( $browser_user_agent, $a_gecko_types[$j] ) ) {
					...
					case 'ie':
						...
						*/
						...
						elseif ( mb_strstr( $browser_user_agent, 'mac') ) {
							$ie_version = 'ieMac';
						}
					...
					case 'op':
						...
						if ( mb_strstr( $browser_number, '9.' ) 
						     && mb_strstr( $browser_user_agent, 'version/' ) ) {
							get_set_count( 'set', 0 );
					...
					*/
					case 'webkit':
						...
						for ( $j = 0; $j < $j_count; $j++ ) {
							if ( mb_strstr( $browser_user_agent, $a_webkit_types[$j] ) ) {
		...
		if ( !$b_success ) {
			...
			*/
			...
			$browser_name = mb_substr( $browser_user_agent, 0, utf8_strcspn( $browser_user_agent , '();') );
			// this extracts just the browser name from the string, if something usable was found
			if ( $browser_name 
			     && utf8_preg_match( '/[^0-9][a-z]*-*\ *[a-z]*\ *[a-z]*/', $browser_name, $a_unhandled_browser ) ) {
	...
	*/
	...
	switch ( $which_test ) {
		...
		case 'header_data':
			...
			if ( !headers_sent() ) {
				if ( mb_stristr( $_SERVER["HTTP_ACCEPT"],"application/xhtml+xml") ) {
...
function get_item_math_number( $pv_browser_number )
{
	...
	if ( $pv_browser_number 
		   && utf8_preg_match( '/^[0-9]*\.*[0-9]*/', $pv_browser_number, $a_browser_math_number ) ) { 
...
function get_os_data ( $pv_browser_string, $pv_browser_name, $pv_version_number  )
{
	...
	*/
	...
	for ( $i = 0; $i < $i_count; $i++ ) {
		...
		!mb_strstr($pv_browser_string, "linux" ) corrects a linux detection bug
		*/
		if ( !is_array( $os_working_data ) 
		     && mb_strstr( $pv_browser_string, $os_working_data ) 
		     && !mb_strstr( $pv_browser_string, "linux" ) ) {
			...
			switch ( $os_working_type ) {
				...
				case 'nt':
					// windows 7
					if ( mb_strstr( $pv_browser_string, 'nt 6.1' ) ) {
						$os_working_number = 6.1;
					}
					// windows vista/server 2008
					elseif ( mb_strstr( $pv_browser_string, 'nt 6.0' ) ) {
						$os_working_number = 6.0;
					}
					// windows server 2003
					elseif ( mb_strstr( $pv_browser_string, 'nt 5.2' ) ) {
						$os_working_number = 5.2;
					}
					// windows xp
					elseif ( mb_strstr( $pv_browser_string, 'nt 5.1' ) 
						|| mb_strstr( $pv_browser_string, 'xp' ) ) {
						$os_working_number = 5.1;//
					}
					// windows 2000
					elseif ( mb_strstr( $pv_browser_string, 'nt 5' ) 
						|| mb_strstr( $pv_browser_string, '2000' ) ) {
						$os_working_number = 5.0;
					}
					// nt 4
					elseif ( mb_strstr( $pv_browser_string, 'nt 4' ) ) {
						$os_working_number = 4;
					}
					// nt 3
					elseif ( mb_strstr( $pv_browser_string, 'nt 3' ) ) {
						$os_working_number = 3;
					}
					break;
				case 'win':
					// windows vista, for opera ID
					if ( mb_strstr( $pv_browser_string, 'vista' ) ) {
						$os_working_number = 6.0;
						$os_working_type = 'nt';
					}
					// windows xp, for opera ID
					elseif ( mb_strstr( $pv_browser_string, 'xp' ) ) {
						$os_working_number = 5.1;
						$os_working_type = 'nt';
					}
					// windows server 2003, for opera ID
					elseif ( mb_strstr( $pv_browser_string, '2003' ) ) {
						$os_working_number = 5.2;
						$os_working_type = 'nt';
					}
					// windows CE
					elseif ( mb_strstr( $pv_browser_string, 'windows ce' ) ) {
						$os_working_number = 'ce';
						$os_working_type = 'nt';
					}
					elseif ( mb_strstr( $pv_browser_string, '95' ) ) {
						$os_working_number = '95';
					}
					elseif ( ( mb_strstr( $pv_browser_string, '9x 4.9' ) ) 
					     || ( mb_strstr( $pv_browser_string, ' me' ) ) ) {
						$os_working_number = 'me';
					}
					elseif ( mb_strstr( $pv_browser_string, '98' ) ) {
						$os_working_number = '98';
					}
					// windows 2000, for opera ID
					elseif ( mb_strstr( $pv_browser_string, '2000' ) ) {
						$os_working_number = 5.0;
						$os_working_type = 'nt';
					}
					break;
				case 'mac':
					if ( mb_strstr( $pv_browser_string, 'os x' ) ) {
		...
		*/
		elseif ( is_array( $os_working_data ) && ( $i == ( $i_count - 2 ) ) ) {
			...
			for ($j = 0; $j < $j_count; $j++) {
				if ( mb_strstr( $pv_browser_string, $os_working_data[$j] ) ) {
...
function get_item_version( $pv_browser_user_agent, $pv_search_string, $pv_b_break_last='', $pv_extra_search='' )
{
	...
	*/
	for ( $i = 0; $i < 4; $i++ ) {
		...
		if ( mb_strpos( $pv_browser_user_agent, $pv_search_string, $start_pos ) !== false ) {
			// update start position if position found
			$start_pos = mb_strpos( $pv_browser_user_agent, $pv_search_string, $start_pos ) + mb_strlen( $pv_search_string );
			/*
			...
			*/
			if ( !$pv_b_break_last 
			     || ( $pv_extra_search && mb_strstr( $pv_browser_user_agent, $pv_extra_search ) ) ) {
				break;
			}
		}
		else
	...
	*/
	...
	$string_working_number = mb_substr( $pv_browser_user_agent, $start_pos, $substring_length );

	// Find the space, ;, or parentheses that ends the number
	$string_working_number = mb_substr( $string_working_number, 0, utf8_strcspn($string_working_number, ' );/') );

	//make sure the returned value is actually the id number and not a string
	// otherwise return ''
	// utf8_strcspn( $string_working_number, '0123456789.') == mb_strlen( $string_working_number)
	//	if ( utf8_preg_match("/\\d/", $string_working_number) == 0 )
 	if ( !is_numeric( mb_substr( $string_working_number, 0, 1 ) ) ) {
		$string_working_number = '';
	}
	//$string_working_number = mb_strrpos( $pv_browser_user_agent, $pv_search_string );
...
function check_is_mobile( $pv_browser_user_agent )
{
	...
	*/
	...
	for ($j = 0; $j < $j_count; $j++) {
		if ( mb_strstr( $pv_browser_user_agent, $a_mobile_search[$j] ) ) {
...
function get_mobile_data( $pv_browser_user_agent )
{
	...
	*/
	...
	for ( $k = 0; $k < $k_count; $k++ ) {
		if ( mb_strstr( $pv_browser_user_agent, $a_mobile_browser[$k] ) ) {
	...
	for ( $k = 0; $k < $k_count; $k++ ) {
		if ( mb_strstr( $pv_browser_user_agent, $a_mobile_device[$k] ) ) {
	...
	for ( $k = 0; $k < $k_count; $k++ ) {
		if ( mb_strstr( $pv_browser_user_agent, $a_mobile_os[$k] ) ) {
	...
	for ( $k = 0; $k < $k_count; $k++ ) {
		if ( mb_strstr( $pv_browser_user_agent, $a_mobile_server[$k] ) ) {
	...
	for ( $k = 0; $k < $k_count; $k++ ) {
		if ( mb_strstr( $pv_browser_user_agent, $a_mobile_tablet[$k] ) ) {
	...
	if ( !$mobile_os 
	     ...
	     && mb_strstr( $pv_browser_user_agent, 'linux' ) ) {
		$mobile_os = 'linux';
