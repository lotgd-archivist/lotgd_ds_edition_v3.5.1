...
if ($_GET['op']=='')
{
	...
	{
		output ('`wErneut verspürst du diesen starken Ruck an deiner Angel und vertrauensvoll stürzt du dich in die Tiefe. Mit dem `#Mal des Wassers`w bist du hier geschützt und gelangst unbehelligt zum Wasserschrein. Dort verbringst du einige Zeit und sammelst neue Kraft.`nDu erhältst `^2 Waldkämpfe`&!');
...
if ($_GET['op']=='zieh1')
{
	output ('`&Du hältst die Angelrute fest in beiden Händen und kommst dem Wasser immer näher. Nach einem kräftigen Ruck landest du schließlich im Wasser und wirst in die Tiefe gezogen.
	`nDu schaffst es nicht, noch einmal kräftig Luft zu holen und verlierst Lebenspunkte. Du rast der schwarzen Tiefe des Sees entgegen. Noch könntest du loslassen und dich retten.');
	$chance=e_rand(4,9);
	$chance=$chance*0.1;
	$session['user']['hitpoints']*=$chance;
	addnav ('Loslassen','fish.php');
	addnav ('Festhalten','watershrine.php?op=shrine');
}
if ($_GET['op']=='shrine')
{
	output ('`&Schließlich lässt das Ziehen nach und du erkennst, dass du in einer Höhle auf dem Grund des Sees bist. Zum Glück kannst du hier atmen.`nDoch bevor du dich deiner Rettung freuen kannst, erblickst du schon einen großen, belebten Wirbel aus Wasser, der sich dir bedrohlich nähert.');
...
if ($battle)
{
	...
	elseif($defeat)
	{
		output('Du wurdest vom Wasserelementar weggespült!`n`4Du bist tot.`nDu verlierst 10% deiner Erfahrung und all Dein Gold.`n');
		$session['user']['gold']=0;
		$session['user']['experience']=round($session['user']['experience']*0.9,0); $session['user']['alive']=false;
...
if ($_GET['op']=='watershrine')
{
	...
	case 3 :
    	...
    	debuglog('hat das Mal des Wassers erlangt.');
		$Char->setBit(CHOSEN_WATER,'marks',1);
