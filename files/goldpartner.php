...
function get_match($rowa,$idb)
{
	...
	if ($rowa['sex']==$rowb['lookingfor'])
	{
		$match_count++;
	}
	if ($rowa['lookingfor']==$rowb['sex'])
	{
		$match_count++;
	}
...
if ($_GET['op']=="create")
{
	$shortname = utf8_preg_replace("([^[:alpha:] _-])","",$_POST['name']);
	...
	if (!$blockaccount)
	{
		...
		else
		{
			$sql = "INSERT INTO goldpartner
                        ...
                        '".db_real_escape_string($shortname)."',
                        '".db_real_escape_string($_POST['sex'])."',
                        '".$session['user']['sex']."',
                        '".$session['user']['acctid']."',
                        '".db_real_escape_string($_POST['desc'])."',
                        '".db_real_escape_string($_POST['quest1'])."',
                        '".db_real_escape_string($_POST['quest2'])."',
                        '".db_real_escape_string($_POST['quest3'])."',
                        '".db_real_escape_string($_POST['quest4'])."',
                        '".db_real_escape_string($_POST['quest5'])."',
                        '".db_real_escape_string($_POST['quest6'])."',
                        '".db_real_escape_string($_POST['quest7'])."',
                        '".db_real_escape_string($_POST['quest8'])."',
                        '".db_real_escape_string($_POST['quest9'])."'
                        )";
			db_query($sql);
...
if ($_GET['op']=="search")
{
	...
	else
	{
		...
		for ($i=0; $i<$max; $i++)
		{
			...
			output("<a href='goldpartner.php?op=seedesc&char=".rawurlencode($row['name'])."&ret=".URLEncode($_SERVER['REQUEST_URI'])."'>".$row['name']."0",true);
...
if ($_GET['op']=="matching")
{
	...
	for ($i=$res-1; $i>-1; $i--)
	{
		...
		{
			output("<a href=\"goldpartner.php?op=write&ret=".URLEncode($_SERVER['REQUEST_URI'])."&to=".rawurlencode($row2[$i]['name'])." \"><img src='./images/newscroll.GIF' width='16' height='16' alt='Mail schreiben' border='0'></a>",true);
			addnav("","goldpartner.php?op=write&ret=".URLEncode($_SERVER['REQUEST_URI'])."&to=".rawurlencode($row2[$i]['name']));
			output("<a href='goldpartner.php?op=seedesc&char=".rawurlencode($row2[$i]['name'])."&ret=".URLEncode($_SERVER['REQUEST_URI'])."'>",true);
...
<input type='radio' name='quest2' value='0' ".($_POST['quest2']==0?'checked="checked"' : '')." >...ein demütiger Mönch.`n
`n",true);
...
`n",true);
	...
	output("`n`^Zuletzt kannst du dich noch kurz (200 Zeichen) beschreiben:`0`n
...
if ($_GET['op']=="change")
{
	$sql = "SELECT Name,description FROM goldpartner WHERE acctid=".$session['user']['acctid'];
	$result = db_query($sql);
	$row = db_fetch_assoc($result);
	output('`&Du stehst in der Kartei als '.$row['Name'].'.
	`n`&Deine bisherige Beschreibung sieht folgendermaßen aus:`n`n'.$row['description'].'`n`n');
	output("`0<form action=\"goldpartner.php?op=preview\" method='POST'>",true);
	output("`n`^Du kannst deine Beschreibung (200 Zeichen) für 500 Gold hier ändern:`0`n
<input name='desc' size='80' maxlength='200' value='");
