...
by Salator: Source vom Liveserver am 12.8. angesehen und kopiert */
	...
	switch( e_rand( 1, 2500) ){
		...
		case 2327:
			...
			if( ($session['user']['mazeturns'] > 5 && e_rand(1,23)%2) || $session['user']['marks'] >= 31 ){
				...
				$out .= "Du weißt jedoch nicht, was du mit dieser Kritzelei anfangen sollst und schmeißt den Zettel achtlos weg.";
			}
		break;
		case 2328:
		case 2329:
		...
		case 2438:
		case 2439:
			$out  = "Eine Schlange beißt dir in die Wade und schlängelt sich zurück in die Büsche.`n";
		...
		case 2445:
			...
			if(e_rand(1,4)==3){
				...
				$out .= "Als du wieder zu dir kommst, fasst du dir an den Kopf und bemerkst eine Wunde.`n";
				$out .= "Du schaust dich um und merkst, wie sich ein Schleier des Vergessens über dich gelegt hat.";
				$session['user']['hitpoints']         = ceil($session['user']['hitpoints'] * 0.65);
				$session['user']['maze_visited']     = implode("", array_fill(0, 144, 0));
				$session['user']['maze_visited'][0]  = 1;
				$session['user']['maze_visited'][ $g_pos ] = 1;
			}
			else{
				$out .= ", drehst dich um und siehst, wie ein kleiner Maulwurf herausguckt und lachend quiekt.";
			}
		break;
		case 2446:
		case 2447:
...
if($_GET['init']==1){
	...
	if( !empty($_GET['superuser']) ){
		if($_GET['superuser']=="showmap"){
			debuglog('nutzte die Superuser-Karte im Schlossgarten');
			$session['user']['maze_visited'] = implode("", array_fill(0, 144, 1));
		}
		elseif($_GET['superuser']=="leave"){
			debuglog('nutzte den Superuser-Exit aus dem Schlossgarten');
	...
	if( $g_pos == 143 ){//ende
		...
		$out = "`n`n`2Du findest einen Goldtopf in dem sich `^".$gold."`2 Goldstücke und `^".$gems."`2 Edelsteine befinden.`n`n";
		...
		if( $gold || $gems ){
			...
			$out .= '`%Da du nicht materiell reich wurdest, bekommst du für deine Anstrengungen `4`b1`b`% Erfahrungspunkt!';
			$session['user']['experience']++;
		}
		// END GILDENMOD

		user_set_stats(array('gardenmaze_visits' => 'gardenmaze_visits+1'));

		$session['user']['maze_visited'] = implode("", array_fill(0, 144, 1));
		$out.= maze_draw(); //by Salator: Labyrinth-Komplettausgabe
		...
		elseif( $session['user']['hitpoints'] < 1 ){
			...
			if( $access_control->su_check(access_control::SU_RIGHT_CASTLECHOOSE) ){
				addnav("Besonderes");
				addnav("göttlicher Überblick", "gardenmaze.php?superuser=showmap&pos=".$g_pos,false,false,false,false,'Diese Aktion gilt als Cheaten und wird geloggt');
				addnav("göttliche Heckenschere", "gardenmaze.php?superuser=leave",false,false,false,false,'Diese Aktion bringt dich hier OHNE LOHN raus und wird geloggt');
				//addnav("Specialstest");
