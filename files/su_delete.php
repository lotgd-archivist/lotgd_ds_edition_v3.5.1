...
switch($str_op) {
	...
	case '':	
		...
		if(empty($_GET['ids']) || !is_array($_GET['ids'])) {
			...
			//fix bathi
			$sql = 'SELECT login,acctid,emailaddress FROM accounts WHERE acctid IN ('.db_intval_in_string($str_ids).')';
			$res = db_query($sql);
		...
		while($a = db_fetch_assoc($res)) {
			...
			if( user_delete($a['acctid']) ) {
				...
				if( is_email($a['emailaddress']) ) {
					send_mail($a['emailaddress'], getsetting('townname','Atrahor').' - Account gelöscht',
				...
				if(!empty($_POST['bl_login'][$a['acctid']])) {
					$str_login = trim(
									db_real_escape_string(
										stripslashes(
											mb_strtolower($_POST['bl_login'][$a['acctid']])
										)
									)
								);
					...
					$sql = 'INSERT INTO blacklist SET value="'.$str_login.'",type='.BLACKLIST_LOGIN.',remarks="Anlässl. Löschung des Spielers '.db_real_escape_string($a['login']).'"';
					db_query($sql);
				}
				if(!empty($_POST['bl_mail'][$a['acctid']])) {
					$str_mail = trim(
									db_real_escape_string(
										stripslashes(
											mb_strtolower($_POST['bl_mail'][$a['acctid']])
										)
									)
								);
					...
					$sql = 'INSERT INTO blacklist SET value="'.$str_mail.'",type='.BLACKLIST_EMAIL.',remarks="Anlässl. Löschung des Spielers '.db_real_escape_string($a['login']).'"';
					db_query($sql);
				}
