...
else if ($_GET['op']=="make")
{
	...
	{ //Stierkopfgebilde
		$session['doll']='art';
		if (item_count(' i.tpl_id="trph" AND owner='.$session['user']['acctid'].' AND name LIKE "%von stier%"') <14)
		$break=1;
	}
	else
	{ //normale Puppe
...
else if ($_GET['op']=="make2")
{
	...
	if($session['doll']!='art' && mb_strpos($name,'Kopf von Stier')) $itemnew['name']='Minotauren-Kadaverpuppe';
