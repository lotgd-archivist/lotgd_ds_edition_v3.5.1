...
if ($_GET['op']=="takeit")
{
	...
	{
		output("`3Die Luft ist rein und du greifst dir das Ei. Sofort spürst du, dass dir dieses Ei einigen Türen öffnen wird und magische Fähigkeiten hat, die sogar den Tod besiegen können. 
		`nAber dir ist auch klar, dass dieses Ei den Neid vieler anderer Krieger auf sich ziehen wird. Auch eine Immunität wird dich nicht vor feindlichen Angriffen schützen, so lange du es besitzt.");
...
else
{
	...
	{
		output("`3Mitten im dichten Wald entdeckst du auf einer starken Astgabel eines gewaltigen Baumes ein riesiges Nest.
		`nDu bemerkst, dass es leer ist und schenkst ihm keine weitere Aufmerksamkeit.`n`n ");
		$session['user']['specialinc']="";
	}
	else if (getsetting("hasegg",0)==0)
	{
		output("`3Mitten im dichten Wald bemerkst du etwas von oben durch die Blätter funkeln. Es ist nicht die Sonne! In deiner Neugier kommst du näher. Du siehst auf einer starken Astgabel eines gewaltigen Baumes ein riesiges Nest - in dem ein großes goldenes Ei funkelt!
		`nDa du keine Ahnung hast, welches Tier solche Eier legt und so riesige Nester baut, bist du unschlüssig, ob du schnell nach oben klettern und das Ei mitnehmen sollst, oder es doch lieber dort lassen solltest. 
		Einerseits könnte das Ei magische Kräfte haben, oder zumindest eine reichliche Mahlzeit bieten - andererseits aber könntest du ebenso verlockend und lecker auf die Mutter dieses Eis wirken...");
	...
	else
	{
		...
		if ($owner == $row['acctid'])
		{
			...
			Es ist die Greifenmutter, die ihr Ei bei dir wittert!! Du schaffst es gerade noch, ihr das Ei entgegenzustrecken, bevor sie über dich herfällt.");
			$lvl = $session['user']['level'];
			...
			if ($session['user']['hitpoints']<=0)
			{
				output("`4Du bist `bTOT`b!!!
				`nDu verlierst glücklicherweise weder Gold noch Erfahrungspunkte.
				`nDu kannst morgen wieder kämpfen.");
				addnav("Zu den Schatten","shades.php");
				addnav("Zu den News","news.php");
				$text .= " und starb dabei";
                CQuest::died();
			}
			addnews($text.".");
		...
		{
			output("`3Mitten im dichten Wald entdeckst du auf einer starken Astgabel eines gewaltigen Baumes ein riesiges Nest.
			`nDu bemerkst, dass es leer ist und schenkst ihm keine weitere Aufmerksamkeit.
			`n`n`^Das goldene Ei befindet sich zur Zeit im Besitz von ".$row['name']."`^!
			`n`3Willst du es ".($row['sex']?"ihr":"ihm")." nicht mal abnehmen?`n");
		}
		$session['user']['specialinc']="";
	}
}
