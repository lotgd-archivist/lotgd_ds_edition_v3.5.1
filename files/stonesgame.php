...
output("`c`b`fS`&t`seinchenglücksspi`&e`fl`0`b`c`n`n");
if ($_GET['op'] == ""){
	output("`fD`&u `sfragst nach dem Glückspiel. Der alte Mann schaut dich ernst an und macht dir klar, dass er dieses Spiel nur um Edelsteine spielen wird!`n");
	output("Dann erklärt er dir das Spiel:`n");
	output("Er wird nacheinander 3 Steine aus seiner Tasche ziehen. Folgende Gewinne sind mögli`&c`fh:`n`#");
...
}else if ($_GET['op'] == "play" && $session['user']['gems']>0){
	output("`sDu wirfst einen Edelstein auf den Tisch und der Alte fängt an, Steine aus seinem Beutel fallen zu lassen.`n`n");
	$session['user']['gems']-=1;
	...
	if ($stonetotal == 2 or $stonetotal == 7){
		//push
		$session['user']['gems']+=1;
		output("`sOhne ein weiteres Wort schiebt dir der alte Mann deinen Edelstein zurück, welchen du schnell schnappst und wieder sicher verstaust.`n");
	}elseif ($stonetotal == 10 or $stonetotal == 11){
		//double
		$session['user']['gems']+=2;
		output("`sOhne ein weiteres Wort legt der alte Mann einen Edelstein zu deinem dazu und schiebt dir die beiden Edelsteine zu. Schnell verstaust du sie sicher in deinem Edelsteinbeutelchen.`n");
	}elseif ($stonetotal == 3 or $stonetotal == 8){
		//triple
		$session['user']['gems']+=4;
		output("`sOhne ein weiteres Wort legt der alte Mann 3 Edelsteine zu deinem dazu und schiebt dir die 4 Edelsteine zu. Schnell verstaust du sie sicher in deinem Edelsteinbeutelchen.`n");
	}elseif ($stonetotal == 15 or $stonetotal == 16){
		//quad
		$session['user']['gems']+=8;
		output("`sMit betrübtem Gesicht, aber immer noch ohne Worte, legt der alte Mann 7 Edelsteine zu deinem dazu und schibt sie dir entgegen. Schnell verstaust du sie sicher in deinem Edelsteinbeutelchen.`n");
	}else{
		output("`sDer Alte schnappt sich deinen Edelstein und lässt ihn verschwinden. Dann schaut er dich erwartungsvoll an.`n");
	}
	addnav("Nochmal","stonesgame.php?op=play");
}else{
	output("`sDu hast keine Edelsteine dabei. Wie peinlich.`n");
}
...
else addnav("Zurück in die Stadt","village.php");
